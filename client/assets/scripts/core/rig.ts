...
export function computePose(action: Action, dir: Dir8, t: number, progress: number): Pose {
  ...
  const pose: Pose = {
    ...
    legSwing: 0, armSwing: 0, skirtSway: 0, skirtTrail: 0,
  ...
  switch (action) {
    ...
    default: {
      ...
      else reach = 14 + Math.sin(p * Math.PI) * 16;

      const pivotX = pose.lean * 0.5 + fx * 3;
      const pivotY = 46;
      pose.attack = {
  ...
  if (pose.sitting) {
    ...
    pose.beltY = 42 + pose.bob + pose.breath * 0.4;
    pose.shoulderY = 64 + pose.bob + pose.breath;
    pose.headX = fx * 4 + pose.lean;
    pose.headY = 78 + pose.bob + pose.breath;
  }
  return pose;
}
